            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('petugas.update', $petugas->id_petugas) }}" method="POST">
                        ...
                        <button type="submit" class="btn btn-green text-white" style="width: 100%">UPDATE</button>
                    </form>
                    @if ($petugas->id_petugas != 1)
                    <form action="{{ route('petugas.destroy', $petugas->id_petugas) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger mt-2" style="width: 100%" onclick="return confirm('Apakah Anda Yakin Ingin Mengahapus Data ini?')">HAPUS</button>
                    </form>
                    @endif
                </div>
            </div>
